...
def sample_contract(db):
    ...
    with db.engine.connect() as conn:
        ...
        conn.execute(text("""
            ...
            (contract_id, venue, venue_market_id, title, outcome_type, outcomes, status, created_time, available_time, source_id)
            VALUES (:contract_id, 'polymarket', 'test_market_1', 'Test Market', 'binary', '["YES", "NO"]', 'active', NOW(), NOW(), :source_id)
    ...
    def test_build_snapshot_basic(self, db, sample_contract):
        ...
        with db.engine.connect() as conn:
            ...
            conn.execute(text("""
                ...
                (contract_id, ts, outcome, price_raw, price_normalized, event_time, available_time)
                VALUES (:contract_id, NOW(), 'YES', 55.0, 0.55, NOW(), NOW())
        ...
        with db.engine.connect() as conn:
            ...
            for i, minutes_ago in enumerate([60, 30, 10]):
                ...
                conn.execute(text("""
                    INSERT INTO cur_contract_prices 
                    (contract_id, ts, outcome, price_raw, price_normalized, event_time, available_time)
                    VALUES (:contract_id, :ts, 'YES', :price, :price_norm, :ts, :ts)
                """), {
                    "contract_id": str(sample_contract),
                    "ts": ts,
                    "price": price,
        ...
        with db.engine.connect() as conn:
            ...
            for i in range(5):
                ...
                conn.execute(text("""
                    ...
                    (contract_id, trade_id, ts, price, size, side, event_time, available_time)
                    VALUES (:contract_id, :trade_id, :ts, :price, :size, 'buy', :ts, :ts)
                """), {
                    "contract_id": str(sample_contract),
                    "trade_id": f"trade_{i}",
                    "ts": ts,
