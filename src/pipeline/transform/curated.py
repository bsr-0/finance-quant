...
class CuratedTransformer:
    ...
    def transform_prices_ohlcv(self) -> int:
        ...
        with self.db.engine.connect() as conn:
            ...
            """), {
                "close_time": close_time, "exchange_tz": exchange_tz,
        ...
        with self.db.engine.connect() as conn:
            result = conn.execute(text("""
                ...
                SELECT
                    ...
                    NOW()                                  AS ingested_at,
                    CASE
                        WHEN r.open < 0 OR r.high < 0 OR r.low < 0 OR r.close < 0
                            THEN 'negative_price'
            ...
            result = conn.execute(text("""
                ...
                factors AS (
                    SELECT
                        pb.*,
                        LAG(pb.close)
                            OVER (PARTITION BY pb.symbol_id ORDER BY pb.date) AS prev_close
                    FROM price_base pb
                ),
                returns AS (
                    SELECT
