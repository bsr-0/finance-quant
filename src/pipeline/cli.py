...
@app.command()
def extract(
    source: str = typer.Argument(..., help="Source to extract (fred, gdelt, polymarket, prices, factors)"),  # noqa: E501
...
@app.command()
def load_raw(
    source: str = typer.Argument(..., help="Source to load (fred, gdelt, polymarket, prices, factors)"),  # noqa: E501
...
def transform_curated():
    ...
    try:
        ...
        critical = [a for a in report.get("alerts", []) if a.get("severity") == Severity.CRITICAL.value]  # noqa: E501
        if critical:
...
def orderbook_snapshots(
    interval: str | None = typer.Option(None, "--interval", "-i", help="Snapshot interval (e.g., 1m, 5m, 1h, off)"),  # noqa: B008, E501
    iterations: int = typer.Option(1, "--iterations", "-n", help="Number of iterations (0 = forever)"),  # noqa: B008, E501
    retention_days: int = typer.Option(30, "--retention-days", help="Retention window in days"),  # noqa: B008
    transform: bool = typer.Option(True, "--transform/--no-transform", help="Transform snapshots to curated"),  # noqa: B008, E501
...
def evaluate(
    ...
    probs_path: Path | None = typer.Option(None, "--probs", help="Prediction market probs file path"),  # noqa: B008, E501
    ...
    if factors_from_db:
        ...
        if db.table_exists("cur_factor_returns"):
            factors_df = pd.DataFrame(db.run_query("SELECT * FROM cur_factor_returns ORDER BY date"))  # noqa: E501
