...
class DataQualityMonitor:
    ...
    def check_freshness(
        self,
        table_name: str,
        timestamp_col: str = "available_time",
        ...
        if age_hours > max_age_hours:
            ...
            return DataQualityAlert(
                ...
                check_name="freshness",
                severity=severity,
                message=f"Data in {table_name} is {age_hours:.1f} hours stale (threshold: {max_age_hours}h)",
                details={
    ...
    def check_completeness(
        ...
        if incomplete_cols:
            return DataQualityAlert(
                ...
                message=f"Low completeness in {table_name}: {len(incomplete_cols)} columns below threshold",
    ...
    def check_price_anomalies(
        self,
        table_name: str = "cur_prices_ohlcv_daily",
    ...
    def check_survivor_bias(
        ...
        if delisted_pct < min_delisted_pct:
            return DataQualityAlert(
                ...
                severity=Severity.WARNING,
                message=f"Potential survivor bias: only {delisted_pct:.1f}% delisted (min: {min_delisted_pct}%)",
                details={
                    "total_symbols": total,
                    "delisted": delisted,
