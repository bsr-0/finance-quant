...
class IntradayRiskMonitor:
    ...
    def record_pnl(self, pnl_delta: float) -> str:
        ...
        if drawdown_pct <= -self.limits.max_drawdown_pct or s.realised_pnl <= -self.limits.max_daily_loss:  # noqa: E501
            if not s.throttled:
                s.throttled = True
                logger.warning(
...
class PreTradeChecker:
    ...
    def check(
        ...
        if notional > lim.max_order_notional:
            return OrderCheckResult(
                status=CheckStatus.REJECTED,
                reason=f"Order notional {notional:,.0f} exceeds limit {lim.max_order_notional:,.0f}",  # noqa: E501
