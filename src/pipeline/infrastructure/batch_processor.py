...
class BatchInserter(Generic[T]):
    ...
    def __init__(self, table_name: str, columns: list[str], config: BatchConfig | None = None):
...
class StreamingBatchProcessor(Generic[T]):
    ...
    def __init__(self, processor: Callable[[list[T]], int], config: BatchConfig | None = None):
